...
Build a reliable home-LAN media manager for DJI footage. A Windows laptop reads an SD card and performs storage-heavy or compute-heavy work when it is online. An always-on Mac hosts the control plane and user interface. A Linux OpenMediaVault server stores originals and generated media.

The system must make interrupted work recoverable and must never turn an automated classification into deletion. The SD card is a read-only source from the application's perspective.
...
The Windows worker connects outbound to the Mac API, registers its identity and capabilities, emits heartbeats, claims eligible jobs, and reports progress and results. It owns SD discovery, inventory, copying, hashing, ffprobe/ffmpeg execution, later analysis, and later export generation.

The worker is allowed to read the configured source root and write only beneath the configured OMV destination root. The source abstraction intentionally exposes no write, rename, move, or delete operations.
...
1. The Windows worker discovers candidate removable sources and reads the selected source recursively.
2. It produces an inventory of case-insensitive MP4/SRT files, explicit missing pairs, sizes, timestamps, and a deterministic source fingerprint.
...
12. The UI may display that the card can be formatted manually only when the configured redundancy policy is satisfied. This message never authorizes deletion of INBOX content.
...
Phase 1 delivers Windows source discovery, inventory, human confirmation on the Mac, leased ingest jobs, OMV preflight, chunked partial copies, safe resume and reconciliation, independent source/destination SHA-256, duplicate and conflict handling, atomic manifests, and the conditional manual-card-release notice.

Acceptance requires repeat imports to be idempotent, divergent content never to be overwritten, and Windows, Mac, or OMV interruption never to create a false `VERIFIED` state.
...
The test portfolio includes platform-independent unit tests, real temporary SQLite migration tests, HTTP contract tests, simulated SD and OMV filesystems, injected network and filesystem failures, Windows-specific reparse-point and atomic-promotion tests, and a manual Mac-to-Windows-to-OMV smoke test at each phase checkpoint.
...
Proxy parameters, media analysis algorithms, review UI details, retention approval semantics, quarantine periods, and iCloud export behavior remain governed by later phases of the canonical specification. They must not be pulled into Phase 0 or Phase 1.
