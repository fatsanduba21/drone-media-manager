...
from drone_media_manager.api.schemas.ingests import IngestResponse
...
from drone_media_manager.api.schemas.sources import SnapshotResponse
...
class WorkerApiClient:
    ...
    def register(
        self, worker_name: str, capabilities: list[str]
    ) -> WorkerRegistrationResponse:
        return self._post(
            "/api/workers/register",
            WorkerRegistration(name=worker_name, capabilities=capabilities),
            WorkerRegistrationResponse,
        )
    ...
    def create_snapshot(
        ...
        expires = expires_at or datetime.now(UTC) + timedelta(hours=24)
        ...
        return self._parse_response(response, SnapshotResponse)

    def append_snapshot_entries(
        self,
        snapshot_id: str,
        worker_id: str,
        revision: int,
        ...
        response = self._request(
            "POST",
            f"/api/sources/snapshots/{snapshot_id}/entries",
            {"worker_id": worker_id, "revision": revision, "entries": list(entries)},
        )
        return self._parse_response(response, SnapshotResponse)

    def finalize_snapshot(
        self, snapshot_id: str, worker_id: str, revision: int, declared_item_count: int
    ) -> SnapshotResponse:
        response = self._request(
            "POST",
            f"/api/sources/snapshots/{snapshot_id}/finalize",
            {
                "worker_id": worker_id,
                "revision": revision,
    ...
    def get_ingest(self, ingest_id: str) -> IngestResponse:
        response = self._request("GET", f"/api/ingests/{ingest_id}", {})
        return self._parse_response(response, IngestResponse)

    def claim(self, worker_id: str, lease_seconds: int = 60) -> ClaimResponse | None:
        request = ClaimRequest(
            worker_id=worker_id, idempotency_key=uuid4(), lease_seconds=lease_seconds
        )
        response = self._request(
            "POST", "/api/worker-jobs/claim", request.model_dump(mode="json")
        )
    ...
    def _mutate(
        self,
        job_id: str,
        request: ProgressRequest | CompleteRequest | FailRequest,
        action: str,
    ) -> JobMutationResponse:
        return self._post(
            f"/api/worker-jobs/{job_id}/{action}", request, JobMutationResponse
        )
    ...
    def _post(
        ...
        return self._parse_response(
            self._request("POST", path, request.model_dump(mode="json")), response_type
        )
    ...
    def _request(self, method: str, path: str, json: dict[str, Any]) -> httpx.Response:
        try:
            ...
            raise WorkerTransportError(
                "control plane request deadline exceeded"
            ) from error
